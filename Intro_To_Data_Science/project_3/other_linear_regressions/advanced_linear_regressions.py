...
def predictions(weather_turnstile):
    ...
    variables = weather_turnstile[['precipi', 'Hour', 'maxtempi', 'minpressurei', 'meandewpti']]
    m = len(variables)
    
    #normalization
    variables, mu, sigma = normalize_features(variables)
    variables['ones'] = np.ones(m)
    
        
    results = smf.ols(formula = "ENTRIESn_hourly ~ C(UNIT) + precipi + C(Hour) + \
                    maxtempi + minpressurei + meandewpti", data = weather_turnstile).fit()
